...
function commonFields(isNew) {
  return [
    ...(isNew ? [
      { name: 'handle', label: 'Nickname di gioco (handle)', required: true, placeholder: 'es. MaxP_TM', hint: 'Nome pubblico @handle + telemetria' },
      { name: 'platform', label: 'Piattaforma', type: 'select', options: [
        { value: '', label: '— Qualsiasi —' }, { value: 'steam', label: 'Steam' },
        { value: 'playstation', label: 'PlayStation' }, { value: 'xbox', label: 'Xbox' }, { value: 'origin', label: 'EA / Origin' },
      ] },
    ] : []),
    ...
    <tr>
      <td>
        <div class="driver-cell">
          ...
          <div>
            <div class="dc-name">${esc(u.display_name || u.handle || '—')}</div>
            <div class="dc-sub">${u.handle ? `@${esc(u.handle)}` : '—'}${u.reserve_driver ? ` · 🤖 ${esc(u.reserve_driver)}` : ''}</div>
          </div>
        </div>
      </td>
...
async function render(root) {
  ...
  root.querySelectorAll('[data-edit]').forEach((b) => b.addEventListener('click', async () => {
    ...
    const ok = await formModal({
      title: `Modifica: ${u.display_name || u.handle || 'pilota'}`,
  ...
  root.querySelectorAll('[data-del]').forEach((b) => b.addEventListener('click', async () => {
    ...
    if (!(await confirmDialog({
      ...
      message: `"${u.display_name || u.handle || 'pilota'}" e TUTTI i suoi dati (risultati, qualifiche, statistiche) verranno rimossi definitivamente. Operazione irreversibile.`,
